...
def show_detected_objects(img, bounding_box_ids, all_bounding_boxes, class_ids, confidence_values, width_ratio, height_ratio):
    ...
    for index in bounding_box_ids:
        ...
        cv2.rectangle(img, (x, y), (x+w, y+h), (255, 0, 0), 2)
        class_with_confidence = classes[class_ids[index[0]]] + " %" + str(int(confidence_values[index[0]] * 100))
        cv2.putText(img, class_with_confidence, (x, y-10), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.5, (255, 0, 0), 1)
...
while True:
    ...
    show_detected_objects(frame, predicted_objects, bbox_locations, class_label_ids, conf_values, ORIGINAL_WIDTH / YOLO_IMAGE_SIZE, ORIGINAL_HEIGHT / YOLO_IMAGE_SIZE)
